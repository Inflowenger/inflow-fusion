...
**Inflow engine instance(s).** The execution runtime. Given a `ProcessRequest` (start node, flow id, context id, timeouts), an engine walks the compiled node graph, asking your backend for whatever it doesn't already have, and executing each node according to its type (run code, evaluate a rule, call an extrinsic subject, hand off to a plugin, jump to another flow). Multiple engine instances can be registered with infra; `inflow-fusion` round-robins new process requests across them (see `inflow.NewProcess` / `inflow.GetResourceCandid`). A resource can instead be **pinned** — tag it `inflow.PinResourceTag` (`pinned-resource`), add one by hand with `inflow.AddResource`, or call `inflow.PinResource(nameOrUrl)` — and then every dispatch uses only that resource, skipping the round-robin until `inflow.UnpinResource` or the next `ReloadResources`. `inflow.GetResourceCandidList` returns the current live pool (this is what the flomorphic-wapp settings dialog renders).
